...
export default function Header({ page, onChange }: Props) {
  const isSelected = (session: number) => {
    if (page === session) {
      return "transition-colors duration-500 text-purple-500 cursor-pointer"
    }
  ...
  return (
    <div className="flex w-full justify-center z-50 px-32 fixed py-10 md:justify-between">
      <div className="flex">
        <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-800 to-purple-500 bg-clip-text text-transparent cursor-pointer" onClick={() => onChange(0)}>Portifólio</h1>
      </div>
